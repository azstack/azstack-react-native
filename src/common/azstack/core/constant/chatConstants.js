export const CHAT_TYPE_USER = 1;
export const CHAT_TYPE_GROUP = 2;

export const CONVERSATION_DELETED_FALSE = 0;
export const CONVERSATION_DELETED_TRUE = 1;

export const MESSAGE_TYPE_TEXT = 1;
export const MESSAGE_TYPE_STICKER = 2;
export const MESSAGE_TYPE_FILE = 3;
export const MESSAGE_TYPE_LOCATION = 4;
export const MESSAGE_TYPE_GROUP_CREATED = 5;
export const MESSAGE_TYPE_GROUP_INVITED = 6;
export const MESSAGE_TYPE_GROUP_LEFT = 7;
export const MESSAGE_TYPE_GROUP_RENAMED = 8;
export const MESSAGE_TYPE_GROUP_ADMIN_CHANGED = 9;
export const MESSAGE_TYPE_GROUP_PUBLIC_JOINED = 10;

export const MESSAGE_STATUS_SENDING = 0;
export const MESSAGE_STATUS_SENT = 1;
export const MESSAGE_STATUS_DELIVERED = 2;
export const MESSAGE_STATUS_SEEN = 3;
export const MESSAGE_STATUS_CANCELLED = 6;

export const MESSAGE_DELETED_FALSE = 0;
export const MESSAGE_DELETED_TRUE = 1;

export const MESSAGE_FILE_TYPE_UNKNOWN = 0;
export const MESSAGE_FILE_TYPE_IMAGE = 1;
export const MESSAGE_FILE_TYPE_AUDIO = 2;
export const MESSAGE_FILE_TYPE_VIDEO = 3;
export const MESSAGE_FILE_TYPE_EXCEL = 4;
export const MESSAGE_FILE_TYPE_WORD = 5;
export const MESSAGE_FILE_TYPE_POWERPOINT = 6;
export const MESSAGE_FILE_TYPE_PDF = 7;
export const MESSAGE_FILE_TYPE_TEXT = 8;
export const MESSAGE_FILE_TYPE_CODE = 9;
export const MESSAGE_FILE_TYPE_ARCHIVE = 10;