		...
		this.subscriptions.onCalloutStatusChanged = this.coreInstances.EventEmitter.addListener(this.coreInstances.eventConstants.EVENT_NAME_CALLOUT_STATUS_CHANGED, ({ error, result }) => {
			...
			switch (result.status) {
				...
				case this.coreInstances.AZStackCore.callConstants.CALL_STATUS_CALLOUT_STATUS_BUSY:
					...
					break;
				case this.coreInstances.AZStackCore.callConstants.CALL_STATUS_CALLOUT_STATUS_TERMINAL:
					this.setState({
						showButtons: false,
						audios: Object.assign({}, this.state.audios, { ringingOut: false, terminal: true })
					});
					setTimeout(() => {
						this.props.onCallEnded();
					}, 1500);
					break;
				case this.coreInstances.AZStackCore.callConstants.CALL_STATUS_CALLOUT_STATUS_NO_SIGNAL:
					this.setState({
						showButtons: false,
						audios: Object.assign({}, this.state.audios, { ringingOut: false, noSignal: true })
					});
					setTimeout(() => {
						this.props.onCallEnded();
					}, 1500);
		...
		return (
			...
			>
				...
				<Video
					...
					volume={1.0}
					muted={false}
					paused={!this.state.audios.terminal}
					resizeMode='cover'
					repeat={false}
					playInBackground={false}
					playWhenInactive={false}
				...
				<Video
					...
					volume={1.0}
					muted={false}
					paused={!this.state.audios.noSignal}
					resizeMode='cover'
					repeat={false}
					playInBackground={false}
					playWhenInactive={false}
